...
import fr.aldostra.link.mod.AldostraMumbleLinkConstants;
...
public class UpdateData {
    ...
    public void send() {
        ...
        if (!success) {
            errorHandler
                    .handleError(NativeUpdateErrorHandler.NativeUpdateError.ERROR_NOT_YET_INITIALIZED);
        }
    ...
    public void set(Minecraft game) {
        try {
            ...
            fAvatarPosition = new float[]{
                    Float.parseFloat(Double.toString(game.player.getPositionVec().x)),
                    Float.parseFloat(Double.toString(game.player.getPositionVec().z)),
                    Float.parseFloat(Double.toString(game.player.getPositionVec().y))};
    ...
    protected String generateIdentity(Minecraft game, int maxLength) {
        ...
        try {
            ...
            AldostraMumbleMumbleMumbleLinkImpl.LOG.fatal("could not generate identity", e);
        }
    ...
    protected String generateContext(Minecraft game, int maxLength) {
        try {
            ...
            newContext.put(ContextManipulator.ContextKey.DOMAIN,
                    AldostraMumbleLinkConstants.MUMBLE_CONTEXT_DOMAIN_ALL_TALK);
            return newContext.toString();
        } catch (JSONException e) {
            AldostraMumbleMumbleMumbleLinkImpl.LOG.fatal("could not generate context", e);
        }

        return AldostraMumbleLinkConstants.MUMBLE_CONTEXT_DOMAIN_ALL_TALK;
    }
